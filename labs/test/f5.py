        ...
        def collect_all_vms(rpool):
            try:
                ...
                for sub in rpool.resourcePool:
                    log(f"📁 Entering sub-pool: {sub.name}", print_lock)
                    collect_all_vms(sub)
            except Exception as e:
                log(f"⚠️ Error accessing VMs: {e}", print_lock)
        for rp in container_view.view:
            if rp.name == rp_name:
                log(f"✅ Located resource pool: {rp_name}", print_lock)
                collect_all_vms(rp)
                break
        return power_map
    except Exception as e:
        ...
        for group_name, components in grouped_components:
            for name, raw_ip, port, _ in components:
                ...
                try:
                    ...
                    log(f"Running (inside SSH): {cmd}", print_lock)
                    child.sendline(cmd)
