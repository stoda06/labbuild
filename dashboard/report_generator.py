...
def calculate_ram_summary(all_allocations: List[Dict], host_map: Dict) -> Dict[str, float]:
    ...
    for allocation in all_allocations:
        ...
        calculated_ram_for_entry = ram
        if total_pods_for_course > 1:
            calculated_ram_for_entry += (total_pods_for_course - 1) * ram / 2

        for env_key in host_map.keys():
            host_prefix_lower = host_map[env_key].lower().strip()
            if any(host_prefix_lower == vh for vh in virtual_hosts_list):
                allocated_ram_by_env[env_key] += calculated_ram_for_entry
...
def _write_data_row(sheet, row, entry, headers, header_keys, header_pos, is_trainer, use_green_fill, host_map: Dict):
    ...
    for h in header_keys:
        if h == "Start/End Pod":
            ...
            if host_name_for_this_column in virtual_hosts_list:
...
# <<< FUNCTION 3 of 5: REPLACED >>>
# In labbuild/dashboard/report_generator.py

def write_group_summary_boxes(sheet, start_row, header_pos, group_pod_total_formula, group_name, group_end_col, data_start_row, data_end_row):
    # <<< FIX: Explicitly get Students and Vendor Pods columns >>>
    vendor_col = header_pos.get("Vendor Pods")
    students_col = header_pos.get("Students")
    has_pod_summary = vendor_col and students_col and (group_pod_total_formula is not None)
    
    host_keys_in_header = [key for key in header_pos if key in SUMMARY_ENV_ORDER]
    has_ram_summary = bool(host_keys_in_header)
    ...
    if has_pod_summary:
        # <<< FIX: Set merge start and end to the exact columns >>>
        ...
        label_cell.font, label_cell.alignment, label_cell.fill = Font(bold=True, size=20), Alignment(horizontal='center', vertical='center'), ExcelStyle.LIGHT_BLUE_FILL.value
        
        sheet.merge_cells(start_row=value_start, start_column=merge_start, end_row=value_end, end_column=merge_end)
        value_cell = sheet.cell(row=value_start, column=merge_start, value=group_pod_total_formula)
        value_cell.font, value_cell.alignment, value_cell.fill = Font(bold=True, size=20), Alignment(horizontal='center', vertical='center'), ExcelStyle.LIGHT_BLUE_FILL.value
    
    if has_ram_summary:
        for host_key in host_keys_in_header:
...
# ==============================================================================
# In labbuild/dashboard/report_generator.py
# REPLACE the existing unpack_current_allocations function with this one.
...
def unpack_current_allocations(documents, vendor_map, apm_lookup_by_code, location_map, host_to_vcenter_map, ram_lookup_map):
        ...
        for c in doc.get('courses', []):
            for pd in c.get('pod_details', []):
                for p in pd.get("pods", [pd]):
                    ...
                    # <<< FIX: Ensure all hosts are collected, even with inconsistent keys >>>
                    host_val = p.get("host") or p.get("virtual_hosts")
                    if host_val: 
                        # Handle both single string and comma-separated strings
                        for h in host_val.split(','):
                            if h.strip():
                                grouped[key]['hosts'].add(h.strip())
    ...
    for course_code, val in grouped.items():
        ...
        course_hosts = sorted(list(val['hosts']))
        ...
        # <<< FIX: Calculate the vCenter name using the map >>>
        host_str = ", ".join(course_hosts)
        vcenters = set()
        for host in course_hosts:
            if vcenter := host_to_vcenter_map.get(host.lower()):
                 vcenters.add(vcenter)
        vcenter_str = ", ".join(sorted(list(vcenters)))
        
        # <<< FIX: Add 'virtual_hosts' and 'vcenter_name' to the final dictionary >>>
        pod_data = {
            'course_code': course_code, 'us_au_location': determine_us_au_location(host_str),
...
def generate_excel_in_memory(course_allocations: List[Dict], trainer_pods: List[Dict], extended_pods: List[Dict], host_map: Dict) -> io.BytesIO:
    ...
    # The `expanded_data` loop is now correctly removed.
    ...
    for group_name in group_order:
        ...
        pod_total_formula = 0
        if (vendor_pods_col_num := header_pos.get("Vendor Pods")) and data_start_row <= data_end_row:
        ...
        current_row = write_group_summary_boxes(sheet, summary_start_row, header_pos, pod_total_formula, group_name, group_end_col, data_start_row, data_end_row)
...
# <<< FUNCTION 5 of 5: REPLACED >>>
# In labbuild/dashboard/report_generator.py
# REPLACE the existing get_full_report_data function with this one.
...
def get_full_report_data(db):
    try:
        ...
        host_map_for_summary = {
            ...
            if 'host_shortcode' in doc and 'host_name' in doc and doc.get('include_for_build') == 'true'
        }
        logger.info(f"Dynamically built HOST_MAP for summary: {host_map_for_summary}")
        
        # <<< START OF FIX for vCenter >>>
        host_to_vcenter_map = {}
        ...
        for doc in host_docs:
            ...
            # This loop populates the map with multiple variations of hostnames
            for key in ['host_name', 'fqdn', 'host_shortcode']:
                if identifier := doc.get(key):
                    if isinstance(identifier, str):
                        # Add the original identifier (e.g., FQDN or shortcode)
                        host_to_vcenter_map[identifier.lower()] = short_vcenter
                        
                        # Also add the short version of the identifier
                        short_identifier = identifier.split('.')[0]
                        host_to_vcenter_map[short_identifier.lower()] = short_vcenter
        logger.info(f"Built vCenter map with {len(host_to_vcenter_map)} keys.")
        # <<< END OF FIX for vCenter >>>
