...
def list_allocations(vendor_shortcode: str, start_filter: Optional[int] = None, end_filter: Optional[int] = None, test_mode: bool = False):
    ...
    allocations_by_tag = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    ...
    try:
            ...
            for tag_doc in alloc_cursor:
                ...
                for course in courses_in_tag:
                    ...
                    if course_vendor is not None and isinstance(course_vendor, str) and course_vendor.lower() == vendor_shortcode.lower():
                        ...
                        if not course_config_data: # Fetch and cache if not present
                            try:
                        ...
                        for pod_detail in pod_details_list:
                            ...
                            item_data = { "tag": tag, "course_name": course_name, "host": host, "pod_num": pod_num, "class_num": class_num, "item_id": item_id, "vr_check_id": vr_check_id, "is_f5_class": is_f5_class_summary, "nested_pods": pod_detail.get("pods", []) if is_f5_class_summary else [], "course_config_data": course_config_data, "host_details": get_host_by_name(host) if test_mode and host != "Unknown Host" else None }
        ...
        if test_mode:
            ...
            if not tasks_to_run and not items_to_potentially_delete: print("\n--- Test Mode: No VR VMs to check based on config/filters. ---"); valid_items_after_test = all_processed_items
                ...
                max_workers = min(len(tasks_to_run), 10) # Limit threads
                ...
                print("--- Test Mode: VM Checks Complete ---"); tested_items_map = {res["original_item_data"]["item_id"]: res for res in test_results} # Map by item_id might be problematic if not unique across classes/pods
                # Need a more robust way to map results back if item_id isn't globally unique
                # For now, let's iterate results and find original item
                items_found_during_test = set()
                for res in test_results:
                    item = res["original_item_data"]
                    if res["exists"]:
                        valid_items_after_test.append(item)
                        items_found_during_test.add(item['item_id']) # Assuming item_id identifies the tested item
                    else:
                        items_to_potentially_delete.append((item["tag"], item["course_name"], item["pod_num"], item["class_num"]))

                # Add items that were *not* tested (e.g., unknown VR name) back to valid list
                for item in all_processed_items:
                    if item["item_id"] not in tested_items_map and item["host"] != "Unknown Host" and item["host_details"] is not None:
                        valid_items_after_test.append(item)

            else: print("\n--- Test Mode: Only items with unknown hosts marked for deletion. ---"); valid_items_after_test = []
        else: valid_items_after_test = all_processed_items # Not test mode
        ...
        # --- Phase 4: Group valid items and Print Output ---
        if valid_items_after_test:
            output_found = True
            # Re-group the valid items for printing
            allocations_by_tag = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
            for item in valid_items_after_test:
                 tag=item["tag"]; course_name=item["course_name"]; host=item["host"]; pod_num=item["pod_num"]; class_num=item["class_num"]; is_f5_class=item["is_f5_class"]; nested_pods=item["nested_pods"]
                 if is_f5_class:
                     # Apply range filter to nested pods again before adding
                     filtered_nested = [p for p in nested_pods if not filter_active or p in filter_range]
                     existing_class = next((d for d in allocations_by_tag[tag][course_name][host] if d.get("type") == "f5_class" and d.get("class_num") == class_num), None)
                     if not existing_class: allocations_by_tag[tag][course_name][host].append({"type": "f5_class", "class_num": class_num, "pods": filtered_nested})
                     # Note: Merging pods if class already exists isn't strictly necessary if data is clean
                 elif pod_num is not None: # Pod already passed filter
                     if not any(d.get("type") == "pod" and d.get("pod_num") == pod_num for d in allocations_by_tag[tag][course_name][host]):
                         allocations_by_tag[tag][course_name][host].append({"type": "pod", "pod_num": pod_num})

            # Print final grouped output
            for tag, courses in sorted(allocations_by_tag.items()):
                print(f"\n Tag: {tag}")
                for course_name, hosts_data in sorted(courses.items()):
                    print(f"  Course: {course_name}")
                    for host, details in sorted(hosts_data.items()):
                        pod_numbers = sorted([d["pod_num"] for d in details if d["type"] == "pod"]); f5_details_display = sorted([d for d in details if d["type"] == "f5_class"], key=lambda x: x["class_num"]); display_items = []
                        if pod_numbers: # Format pod ranges
                            ...
                            for start, end in ranges: display_items.append(str(start) if start == end else f"{start}-{end}")
                        for f5 in f5_details_display: # Format F5
                            pod_list_str = f"(Pods: {', '.join(map(str, f5['pods']))})" if f5['pods'] else "(Class Only)"
                            display_items.append(f"Class {f5['class_num']} {pod_list_str}")
                        if display_items: print(f"    Host: {host:<15} Pods/Classes: {', '.join(display_items)}")
