// In dashboard/static/js/settings_build_rules.js
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    ruleModal.addEventListener('show.bs.modal', function(event) {
        ...
        // Reset the form for all modes
        form.reset();
        
        if (mode === 'add') {
            ruleModal.querySelector('.modal-title').textContent = 'Add New Build Rule';
            form.action = '/settings/build-rules/add';
            form.querySelector('#rule_id').value = '';
        } else if (mode === 'edit' || mode === 'clone') {
            // --- MODIFICATION: Handle both 'edit' and 'clone' modes ---
            const ruleData = JSON.parse(button.dataset.ruleJson);
            
            if (mode === 'edit') {
                ruleModal.querySelector('.modal-title').textContent = 'Edit Build Rule';
                form.action = `/settings/build-rules/update`;
                form.querySelector('#rule_id').value = ruleData._id;
                form.querySelector('#rule_name').value = ruleData.rule_name || '';
            } else { // mode === 'clone'
                ruleModal.querySelector('.modal-title').textContent = 'Clone Build Rule';
                form.action = '/settings/build-rules/add'; // A clone creates a new rule
                form.querySelector('#rule_id').value = ''; // A clone has no ID yet
                // Prepend to the name to indicate it's a copy
                form.querySelector('#rule_name').value = `Clone of ${ruleData.rule_name || 'Rule'}`;
            }

            // --- Populate fields (common logic for both edit and clone) ---
